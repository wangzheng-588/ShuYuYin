package cn.shuyuyin.common.listener
 ...
 */
class MyAnimatorUpdateListener(var animator: ObjectAnimator) : ValueAnimator.AnimatorUpdateListener {
    ...
    override fun onAnimationUpdate(animation: ValueAnimator) {
        ...
        if (isPause) {
            if (!isPaused) {
                ...
                animation.setInterpolator({ fraction })
                isPaused = true
            }
            //每隔动画播放的时间，我们都会将播放时间往回调整，以便重新播放的时候接着使用这个时间,同时也为了让整个动画不结束
            object : CountDownTimer(ValueAnimator.getFrameDelay(), ValueAnimator.getFrameDelay()) {
